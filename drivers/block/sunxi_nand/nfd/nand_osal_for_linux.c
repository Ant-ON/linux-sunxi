...
{
    //printk(KERN_WARNING "[NAND] nand clk request start\n");
	...
	//printk(KERN_WARNING "[NAND] nand clk request ok!\n");
	return 0;
}
...
void nanddma_buffdone(dma_hdl_t dma_hdle, void *parg)
{
	...
	wake_up( &DMA_wait );
	//printk(KERN_WARNING "buffer done. nanddma_completed_flag: %d\n", nanddma_completed_flag);
}
...
dma_hdl_t NAND_RequestDMA(void)
{
	...
	if(dma_hdle == NULL)
	{
		printk(KERN_WARNING "[NAND DMA] request dma fail\n");
		return dma_hdle;
	}
	dbg_rbint("[NAND DMA] request dma success\n");

	/* set full done callback */
	done_cb.func = nanddma_buffdone;
	done_cb.parg = NULL;
	if(0 != sw_dma_ctl(dma_hdle, DMA_OP_SET_FD_CB, (void *)&done_cb))
		printk(KERN_WARNING "[NAND DMA] set fulldone_cb fail\n");
	dbg_rbint("[NAND DMA] set fulldone_cb success\n");
...
__s32  NAND_ReleaseDMA(void)
{
    if(dma_start_flag==1)
    {
        ...
        if(0 != sw_dma_ctl(dma_hdle, DMA_OP_STOP, NULL))
    		printk(KERN_WARNING "[NAND DMA] stop dma fail\n");
    }

    if(0!= sw_dma_release(dma_hdle))
        printk(KERN_WARNING "[NAND DMA] release dma fail\n");
...
//config dma
	...
	{
		/* config para */
		memset(&dma_nand_config, 0, sizeof(dma_nand_config));
		dma_nand_config.xfer_type.src_data_width 	= DATA_WIDTH_32BIT;
		dma_nand_config.xfer_type.src_bst_len 	= DATA_BRST_4;
		dma_nand_config.xfer_type.dst_data_width 	= DATA_WIDTH_32BIT;
		dma_nand_config.xfer_type.dst_bst_len 	= DATA_BRST_4;
		dma_nand_config.address_type.src_addr_mode 	= DDMA_ADDR_IO;
		dma_nand_config.address_type.dst_addr_mode 	= DDMA_ADDR_LINEAR;
		dma_nand_config.src_drq_type 	= D_DST_NAND;
		dma_nand_config.dst_drq_type 	= D_DST_SDRAM;
		dma_nand_config.bconti_mode 		= false;
		dma_nand_config.irq_spt 		= CHAN_IRQ_FD;
//        printk(KERN_WARNING "nand read config done!");
	}
	else //write to nand
	{
		/* config para */
		...
		dma_nand_config.xfer_type.dst_data_width 	= DATA_WIDTH_32BIT;
		dma_nand_config.xfer_type.dst_bst_len 	= DATA_BRST_4;
		dma_nand_config.address_type.src_addr_mode 	= DDMA_ADDR_LINEAR;
		dma_nand_config.address_type.dst_addr_mode 	= DDMA_ADDR_IO;
		dma_nand_config.src_drq_type 	= D_DST_SDRAM;
		dma_nand_config.dst_drq_type 	= D_DST_NAND;
		dma_nand_config.bconti_mode 		= false;
		dma_nand_config.irq_spt 		= CHAN_IRQ_FD;
//        printk(KERN_WARNING "nand write config done!");
	}

	if(0 != sw_dma_config(dma_hdle, &dma_nand_config))
		printk(KERN_WARNING "[NAND DMA] config dma fail\n");

	{
		...
		{
			printk(KERN_WARNING "[NAND DMA] set dma para fail\n");
		}
	}
...
//start dma
    ...
    {
        dma_start_flag=1;
        dbg_rbint("[NAND DMA] start dma...\n");
        if(0 != sw_dma_ctl(dma_hdle, DMA_OP_START, NULL))
    	{
    		printk(KERN_WARNING "[NAND DMA] start dma fail\n");
    	}
    }
}
